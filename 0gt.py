...
def newinfo(uid):
    try:
        response = requests.get(f"https://akiru-info.vercel.app/info?uid={uid}&region=me")
        ...
        if response.status_code == 200:
            ...
            formatted_basic = {
                ...
                'csRank': basic_info.get('rankingPoints', 0),
                'Exp': basic_info.get('exp', 0)
            }
            
            clan_info = response.get('captainBasicInfo', {})
            if not clan_info:
                clan_info = "false"
                ...
                clan_info = {
                    ...
                    'memberNum': clan_info.get('membersCount', 0)
                }
            
            info = {
                'captainBasicInfo': formatted_basic,
                'clan_info': clan_info,  
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
            f"https://mazid-gmr-like-v2.vercel.app/like?key=MAZIDxBIG-DADDY&uid={uid}&server_name=me"
        )
...
[FFFFFF]عدد الإعجابات المُضافة : [00FF00]{likes_added}  
[FFFFFF]الإعجابات قبل : [00FF00]{likes_before}  
[FFFFFF]الإعجابات بعد : [00FF00]{likes_after}  
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Credits: [FFFFFF]ProjectS [00FF00]FoxX!!
                """
            else:
                message = (
...
def get_time(uid):
    ...
    if "error" in data:
        ...
        time = f"""
[C][B][11EAFD]‎━━━━━━━━━━━━
[FFFFFF]الأيام: [FFA500]{days}
[FFFFFF]الساعات: [32CD32]{hours}
[FFFFFF]الدقائق: [1E90FF]{minutes}
[FFFFFF]الثواني: [FF4500]{seconds}

[FFFFFF]هل تحتاج المزيد من الوقت؟
[00FFFF]تواصل معي على تيليجرام: [FF1493]@illllillilllli
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Credits: ProjectS FoxX!!
        """
        return time
...
def spam_requests(player_id):
    url = f"http://168.231.113.96:8398/spam?id={player_id}"
    res = requests.get(url)
    if res.status_code() == 200:
        print("Done-Spam")
    else:
...
def attack_profail(player_id):
    url = f"http://168.231.113.96:8399/attack?uid={player_id}"
    res = requests.get(url)
    if res.status_code() == 200:
        print("Done-Attack")
    else:
...
def Increase_visits(player_id):
    url = f"https://mazid-gmr-visit.vercel.app/visit?region=ME&uid={player_id}"
    res = requests.get(url)
    if res.status_code == 200:      
        data = res.json()
        uid = data["UID"]
        ...
        msg_visit = f"""
    [C][B][11EAFD]‎━━━━━━━━━━━━
    [FFFFFF]اسم اللاعب : [00FF00]{id_name}
    [FFFFFF]معرّف اللاعب : [00FF00]{uid}
    [FFFFFF]عدد الزيارات : [00FF00]{succvisit}
    [C][B][11EAFD]‎━━━━━━━━━━━━
    [C][B][FFB300]Credits: [FFFFFF]ProjectS [00FF00]FoxX!!
    """
        return msg_visit
...
[11EAFD][B][C]
━━━━━━━━━━━━
[FFFFFF]اسم اللاعب : {id_name}
[FFFFFF]معرّف اللاعب : {uid}
[FFFFFF]منطقة الحساب : {region}
‎━━━━━━━━━━━━
[808080][B][C]Credits: ProjectS FoxX!!
    """
        return msg_visit
    else:
            ...
            if senthi == True:
                ...
                clients.send(
                        self.GenResponsMsg(
                            f"""
[C][B][00FFFF]━━━━━━━━━━━━━━━
[FFFFFF]السلام عليكم، كيف حالك؟
شكراً على قبول طلب الصداقة!
أرسل أي إيموجي لمعرفة ما عليك فعله.
[C][B][00FFFF]━━━━━━━━━━━━━━━
""", idinv
                        )
                )
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""
[C][B][00FFFF]━━━━━━━━━━━━━━━
[FFFFFF]هل ترغب في شراء سيرفر خاص باسمك؟

[FFFFFF]لدعم مشروعك والحصول على سيرفر قوي ومستقر، تواصل مباشرة مع المطور فوكس عبر:

[00FF00]تيليجرام: @illllillilllli
[00FF00]إنستجرام: om1p9

[FFFFFF]رأيت الأوامر؟ أرسل:
[00FF00]/🗿help

[C][B][00FFFF]━━━━━━━━━━━━━━━
[C][B][808080]Credits: ProjectS FoxX!!
""",uid
                            )
                        )
                else:
...
[11EAFD][b][c]
━━━━━━━━━━━━
تم بدأ سبام طلبات الانضمام للاعب:
{iddd_display}
━━━━━━━━━━━━
[808080][b][c]Credits: ProjectS FoxX!!
            """, uid))
...
[11EAFD][b][c]
━━━━━━━━━━━━
اسم اللاعب: {banned_status['player_name']}
معرّف اللاعب: {player_id}
الحالة: {status}
━━━━━━━━━━━━
[808080][b][c]Credits: ProjectS FoxX!!
            """
            ...
            if "1200" in data.hex()[0:4] and b"/attack/" in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    clients.send(self.GenResponsMsg("جاري الهجوم على لاعب......", uid))                   
                    b = attack_profail(player_id)
                    player_id = fix_num(player_id)
                    response_message = f"""
[11EAFD][B][C]
‎━━━━━━━━━━━━
[FFFFFF]الحالة الحالية : ثم نككككح بروفايل لاعب
[FFFFFF]معرّف اللاعب : {player_id}
‎━━━━━━━━━━━━
[808080][B][C]Credits: ProjectS FoxX!!
            """
                    clients.send(self.GenResponsMsg(response_message, uid))
...
[11EAFD][B][C]
‎━━━━━━━━━━━━
[FFFFFF]الحالة الحالية : ثم عمل سبام طلبات صداقة للاعب
[FFFFFF]معرّف اللاعب : {player_id}
‎━━━━━━━━━━━━
[808080][B][C]Credits: ProjectS FoxX!!
            """
            ...
            if "1200" in data.hex()[0:4] and b"/ai" in data:
                try:
                    ...
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]                    
                    clients.send(self.GenResponsMsg("يثم تواصل معا سيرفر رجاء الانتظار...", uid))
                    try:
...
[00FFFF]نقاط الترتيب (باتل رويال): [FFFFFF]{clan_admin_brrank}
	                        ...
	                        if info_response['status'] == "ok":
	                        	...
	                        	if "|" in bio:
	                        	    bio = bio.replace("|"," ")
	                        	br_rank = fix_num(basic_info['csRank'])
...
"""
	                ...
	                response_mssg = f"""
[C][B][00FFFF]━━━━━━━━━━━━
[FFFFFF][B]معرفة السيرفر الخاص باللاعب:

[1E90FF]/🗿region/104[c]145[c]933[c]49

[FFFFFF]ميزات أكثر وأقوى قادمة قريبًا للبوت...  
[00FF00]استمتع باستخدامه بدون تخريب!
[C][B][00FFFF]━━━━━━━━━━━━
"""
	                clients.send(self.GenResponsMsg(response_mssg, uid))
    def parse_my_message(self, serialized_data):
