...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
            f"http://10.50.6.150:4590/like?uid={uid}&key=trickzqw&count=100"
        )
...
def spam_requests(player_id):
    url = f"http://10.50.6.150:8458/spam?id={player_id}"
    res = requests.get(url)
    if res.status_code() == 200:
        print("Done-Spam")
    else:
...
def attack_profail(player_id):
    url = f"http://10.50.6.150:8448/attack?uid={player_id}"
    res = requests.get(url)
    if res.status_code() == 200:
        print("Done-Attack")
    else:
